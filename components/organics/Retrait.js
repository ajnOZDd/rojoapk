...
const Retrait = ({ style }) => {
  ...
  const [drawlDate, setDrawlDate] = useState(new Date());
  ...
  const handleWithdrawal = async () => {
    try {
      const walletRef = ref(database, `wallets/${userId}`);
      // const withdrawalAmount = parseFloat(withdrawalAmount);
      // const snapshot = await get(walletRef);
      ...
      if (wallet.balance >= withdrawalAmount) {
        ...
        const newWithdrawal = {
          ...
          date: drawlDate.toISOString(),
        };

        await update(walletRef, {
          balance: wallet.balance - withdrawalAmount,
          withdrawals: [...wallet.withdrawals, newWithdrawal],
        });
        ...
        Alert.alert('Erreur', 'Solde insuffisant pour effectuer le retrait.');
      }
    } catch (error) {
  ...
  return (
    <View style={[styles.container, style]}>
      ...
      <View style={styles.section}>
        <TextField style={styles.sectionTitle}>Retrait</TextField>
